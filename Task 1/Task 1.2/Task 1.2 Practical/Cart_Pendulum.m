...
function dy = cart_pendulum_dynamics(y, m, M, L, g,  u)
  ...
  cos_theta = cos(y(3));
  dy(1,1) = y(2);
  dy(2,1) = u/(M+m-m*cos_theta^2) + (m*L*(y(4)^2)*sin_theta - m*g*sin_theta*cos_theta)/(M+m-m*cos_theta^2);
  dy(3,1) = y(4);
  dy(4,1) = (-1)*(u*cos_theta - (M+m)*g*sin_theta + m*L*(sin_theta*cos_theta)*y(4)^2)/(m*L*cos_theta^2 - (M+m)*L);
endfunction
...
function [t,y] = pole_place_cart_pendulum(m, M, L, g, y_setpoint, y0)
  ...
  eigs = [-5; -10; -1.2; -1.3];                            ## Initialise desired eigenvalues*
...
function cart_pendulum_main()
  ...
  ##[t,y] = pole_place_cart_pendulum(m, M, L, g, y_setpoint, y0); ##0.00007   -0.00008    3.14161   -0.00001
  ##[t,y] = lqr_cart_pendulum(m, M, L, g, y_setpoint, y0);        ##  0.00007  -0.00013   3.14161  -0.00002
